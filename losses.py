...
def contour(x, kernel = (3,3), stride = 1):
    '''
    Differenciable aproximation of contour extraction
    
    '''   
    min_pool_x = torch.nn.functional.max_pool2d(x*-1, kernel, stride,1)*-1
    max_min_pool_x = torch.nn.functional.max_pool2d(min_pool_x, kernel, stride,1)
...
class contour_loss():
    ...
    def __call__(self, probs: Tensor, target: Tensor, _: Tensor) -> Tensor:
        ...
        b, _, w, h = pc.shape
        target_skeleton = contour(tc, (5,5)).sum(axis=(2,3))
...
class GeneralizedDice():
    ...
    def __call__(self, probs: Tensor, target: Tensor, _: Tensor) -> Tensor:
        ...
        union: Tensor = w * (einsum("bcwh->bc", pc)**2 + einsum("bcwh->bc", tc)**2)
