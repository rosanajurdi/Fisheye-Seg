...
from utils import id_, map_, class2one_hot, one_hot2dist, Class_Select
from utils import simplex, sset, one_hot
#from DataSet_Module.Generate_BB_channels import draw_filter

F = Union[Path, BinaryIO]
D = Union[Image.Image, np.ndarray, Tensor]
import sys
sys.path.append('/home/eljurros/spare-workplace/WoodScape_Segmentation_Project/DataSet_Module')
...
def get_loaders(args, data_folder: str,
    ...
    png_transform = transforms.Compose([
        ...
        lambda nd: np.transpose(nd,(-1,0,1)),  # max <= 1
    ...
    gt_transform = transforms.Compose([
        ...
        lambda nd: Class_Select(nd,C=n_class),
...
class SliceDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> List[Any]:
        ...
        if path_name.suffix == ".png":
            images = [Image.open(files[index]).resize((512,512)) for files in self.files]
        ...
        img, gt = t_tensors[:2]
        import pandas as pd
        from Generate_BB_channels import draw_bbox
        df = pd.read_csv(os.path.join('/home/2017011/reljur01/OMNIDATA','box_2d_annotations',
                                      '{}.txt'.format(filename.split('.png')[0])), sep=",",
                                       header=0,names=['class','instance','x1','x2','y1','y2'])
        class_dict = {'void':0, 'road':1, 'lanemarks':2,'curb':3,
                      'person':4, 'rider': 5,'vehicles':  6,'bicycle':  7,'motorcycle':  8,'traffic_sign':  9 }
        mask = np.ones((10, 966,1280), np.uint8)
        import matplotlib.pyplot as plt
        df.columns = ['class', 'instance', 'x1', 'y1', 'x2', 'y2']
        
        for i, row in df.iterrows():
            if len(np.unique(mask[class_dict[row['class']]]))!=2:
                mask[class_dict[row['class']]]= np.zeros((966,1280), np.uint8)
            mask = draw_bbox(row, mask,class_dict[row['class']])
            #plt.imshow(mask[class_dict[row['class']]])
            #plt.show()
        Bounding_boxes = np.array([np.array(Image.fromarray(mask[i]).resize((512,512))) for i in range(10)]).astype(float)
        #print(img, gt)
        bounds = [f(img, gt, t, filename) for f, t in zip(self.bounds_generators, t_tensors[2:])]

        # return t_tensors + [filename] + bounds
        return [filename] + [Bounding_boxes]+ t_tensors  + bounds
