...
from layers import conv_block, conv_block_3, maxpool, conv_decod_block, DEFconv_block


from layers import upSampleConv, convBatch, residualConv
from dcn_layers import DeformConv2d
#from dcn_2 import DeformConv2d

def upSampleDeformedConv(nin, nout, kernel_size=3, upscale=2, padding=1, bias=False):
    return nn.Sequential(
        nn.Upsample(scale_factor=upscale),
        convBatch(nin, nout, kernel_size=kernel_size, stride=1, padding=padding, bias=bias, layer=DeformConv2d),
...
class Conv_residual_conv(nn.Module):
    def __init__(self, in_dim, out_dim, act_fn, layer = conv_block):
        super().__init__()
        self.in_dim = in_dim
        self.out_dim = out_dim
        act_fn = act_fn

        self.conv_1 = layer(self.in_dim, self.out_dim, act_fn)
        self.conv_2 = conv_block_3(self.out_dim, self.out_dim, act_fn)
        self.conv_3 = conv_block(self.out_dim, self.out_dim, act_fn)
...
class DeformedResidualUNet_Enc1(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32):
        super().__init__()
        self.in_dim = input_nc
        self.out_dim = ngf
        ...
        # Encoder
        self.down_1 = Conv_residual_conv(self.in_dim, self.out_dim, act_fn, layer=DEFconv_block)
        self.pool_1 = maxpool()
        self.down_2 = Conv_residual_conv(self.out_dim, self.out_dim * 2, act_fn)
        self.pool_2 = maxpool()
        self.down_3 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 4, act_fn)
        ...
        self.up_3 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 2, act_fn_2)
        self.deconv_4 = conv_decod_block(self.out_dim * 2, self.out_dim, act_fn_2)
        self.up_4 = Conv_residual_conv(self.out_dim, self.out_dim, act_fn_2, layer=DEFconv_block)
...
class DeformedResidualUNet_EncENC2(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32):
        super().__init__()
        self.in_dim = input_nc
        self.out_dim = ngf
        ...
        self.down_1 = Conv_residual_conv(self.in_dim, self.out_dim, act_fn, layer=DEFconv_block)
        self.pool_1 = maxpool()
        self.down_2 = Conv_residual_conv(self.out_dim, self.out_dim * 2, act_fn, layer=DEFconv_block)
        self.pool_2 = maxpool()
        self.down_3 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 4, act_fn)
        self.pool_3 = maxpool()
        self.down_4 = Conv_residual_conv(self.out_dim * 4, self.out_dim * 8, act_fn)
        ...
        self.up_2 = Conv_residual_conv(self.out_dim * 4, self.out_dim * 4, act_fn_2)
        self.deconv_3 = conv_decod_block(self.out_dim * 4, self.out_dim * 2, act_fn_2)
        self.up_3 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 2, act_fn_2, layer=DEFconv_block)
        self.deconv_4 = conv_decod_block(self.out_dim * 2, self.out_dim, act_fn_2)
        self.up_4 = Conv_residual_conv(self.out_dim, self.out_dim, act_fn_2, layer=DEFconv_block)
...
class DeformedResidualUNet_EncALL(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32):
        super().__init__()
        self.in_dim = input_nc
        self.out_dim = ngf
        ...
        self.down_2 = Conv_residual_conv(self.out_dim, self.out_dim * 2, act_fn, layer=DEFconv_block)
        self.pool_2 = maxpool()
        self.down_3 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 4, act_fn, layer=DEFconv_block)
        self.pool_3 = maxpool()
        self.down_4 = Conv_residual_conv(self.out_dim * 4, self.out_dim * 8, act_fn)
        self.pool_4 = maxpool()
        ...
        self.up_1 = Conv_residual_conv(self.out_dim * 8, self.out_dim * 8, act_fn_2)
        self.deconv_2 = conv_decod_block(self.out_dim * 8, self.out_dim * 4, act_fn_2)
        self.up_2 = Conv_residual_conv(self.out_dim * 4, self.out_dim * 4, act_fn_2, layer=DEFconv_block)
        self.deconv_3 = conv_decod_block(self.out_dim * 4, self.out_dim * 2, act_fn_2)
        self.up_3 = Conv_residual_conv(self.out_dim * 2, self.out_dim * 2, act_fn_2, layer=DEFconv_block)
        self.deconv_4 = conv_decod_block(self.out_dim * 2, self.out_dim, act_fn_2)
        self.up_4 = Conv_residual_conv(self.out_dim, self.out_dim, act_fn_2, layer=DEFconv_block)
...
class DeformedResidualUNet_Decoder(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32):
        super().__init__()
        self.in_dim = input_nc
        self.out_dim = ngf
...
class ResNet_Ours(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32):
        ...
        self.classifier = DeepLabHead(self.out_dim * 8, output_nc, aspp_dilate)
        # Decoder
...
class DeformedResidualUNet_L1(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=32):
        super().__init__()
        self.in_dim = input_nc
        self.out_dim = ngf
...
class DEFORMED_UNet(nn.Module):
    def __init__(self, nin, nout, nG=64):
        super().__init__()

        self.conv0 = nn.Sequential(convBatch(nin, nG, layer=DeformConv2d),
                                   convBatch(nG, nG))
        self.conv1 = nn.Sequential(convBatch(nG * 1, nG * 2, stride=2),
                                   convBatch(nG * 2, nG * 2))
        self.conv2 = nn.Sequential(convBatch(nG * 2, nG * 4, stride=2),
        ...
        self.deconv3 = upSampleConv(nG * 2, nG * 2)
        self.conv7 = nn.Sequential(convBatch(nG * 3, nG * 1),
                                   convBatch(nG * 1, nG * 1, layer=DeformConv2d))
        self.final = nn.Conv2d(nG, nout, kernel_size=1)
...
class DEFORMED_UNet_onelayer(nn.Module):
    def __init__(self, nin, nout, nG=64):
        super().__init__()
...
class BB_Unet(nn.Module):
    def __init__(self, nin, nout, nG=64):
        ...
        self.b1 = BBConv(10, 256, 4)
        self.b2 = BBConv(10, 128, 2)
        self.b3 = BBConv(10, 64, 1)
    ...
    def forward(self, input, bounding_boxes):
        input = input.float()
        bb = bounding_boxes.float()
        x0 = self.conv0(input)
        x1 = self.conv1(x0)
        x2 = self.conv2(x1)
        ...
        y0 = self.deconv1(bridge)
        y1 = self.deconv2(self.conv5(torch.cat((y0, x2_1), dim=1)))
        y2 = self.deconv3(self.conv6(torch.cat((y1, x1_1), dim=1)))
        y3 = self.conv7(torch.cat((y2, x0_1), dim=1))
