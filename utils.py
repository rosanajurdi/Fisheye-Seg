...
def meta_IoU(sum_str: str, label: Tensor, pred: Tensor, smooth: float = 1e-8) -> float:
    assert label.shape == pred.shape
    assert one_hot(label)
    assert one_hot(pred)

    inter: Tensor = einsum(sum_str, [intersection(label, pred)]).type(torch.float32)
    unionn: Tensor = einsum(sum_str, [union(label, pred)]).type(torch.float32)

    dices: Tensor = (inter + smooth) / (unionn + smooth)
...
IoU_coeff = partial(meta_IoU, "bcwh->bc")
...
def Class_Select(seg: Tensor, C: int) -> Tensor:
    seg[seg>=C] = 0
    #print(seg.unique())
...
def save_images(segs: Tensor, names: Iterable[str], root: str, mode: str, iter: int) -> None:
    ...
    for seg, name in zip(segs, names):
        try:
            save_path = Path(root, "buffer", mode, name).with_suffix(".png")
