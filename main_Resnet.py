...
def run(args: argparse.Namespace) -> None:
    ...
    for i in range(n_epoch):
        ...
        df = pd.DataFrame({"tra_loss": metrics["tra_loss"].mean(dim=1).cpu().numpy(),
                           ...
                           'road':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,0], 
                           'lanemarks':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,1],
                           'curb':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,2],
                           'person':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,3],
                           'vehicles':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,4],
                           'rider':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,5],
                           'vehicle':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,6],
                           'bicycle':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,7] , 
                           'motorcycle':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,8],           
                           'traffic-sign':  metrics["val_dice"][:, :, range(0,10)].mean(dim=1).cpu().numpy()[:,9] ,                    
                           })
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument('--dataset', type=str, default='/home/2017011/reljur01/OMNIDATA/FOLD_V4')
    parser.add_argument("--csv", type=str, default= 'metrics.csv')
    parser.add_argument("--workdir", type=str, default = '/home/2017011/reljur01/OMNIDATA/FOLD_V4/results/ResNet_Ours')
    ...
    parser.add_argument("--network", type=str,  help="The network to use", default='ResNet_Ours')
