...
Script to generate Bunding box channels 
'''
'''
...
def circ_mask(shape, c_attr, n_organ):
    ...
    return x
'''
def draw_bbox(bbox_coords, mask,index, shape):
    ...
    if len(bbox_coords) != 1:
        mask[index][np.int(bbox_coords[0]):np.int(bbox_coords[2]),
                np.int(bbox_coords[1]):np.int(bbox_coords[3])] = 1
...
def draw_bbox(bbox,mask, index):
    mask[index][np.int(bbox['y1']):np.int(bbox['y2']), np.int(bbox['x1']):np.int(bbox['x2'])] = 1.00
    return mask
