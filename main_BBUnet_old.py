...
from utils import dice_coef, dice_batch, save_images, tqdm_, haussdorf,IoU_coeff
...
def setup(args, n_class: int) -> Tuple[Any, Any, Any, List[Callable], List[float], Callable]:
    ...
    if args.weights:
        ...
        net = nn.DataParallel(net_class(args.modalities, n_class)).to(device)
...
def do_epoch(mode: str, net: Any, device: Any, loader: DataLoader, epc: int,
    ...
    for j, data in tq_iter: 
        ...
        # Forward
        if net._get_name() == 'BB_Unet':
            pred_logits: Tensor = net(image, bounding_boxes)
        ...
        for loss_fn, label, w, bound in ziped:
            losses = [w * loss_fn(pred_probs, label, bound)]
        ...
        dices: Tensor = IoU_coeff(predicted_mask, target.detach())
        ...
        dsc_dict = {f"IoU{n}": all_dices[big_slice, n].mean() for n in metric_axis}
        hauss_dict = {f"HD{n}": haussdorf_log[big_slice, n].mean() for n in metric_axis} if compute_haussdorf else {}
        batch_dict = {f"bIoU{n}": batch_dices[:j, n].mean() for n in metric_axis} if B > 1 and mode == "val" else {}

        mean_dict = {"IoU": all_dices[big_slice, metric_axis].mean(),
                     "Road": all_dices[big_slice, metric_axis[0]].mean(),
                     "lanemarks": all_dices[big_slice, metric_axis[1]].mean(),
                     "curb": all_dices[big_slice, metric_axis[2]].mean(),
                     "person": all_dices[big_slice, metric_axis[3]].mean(),
                     "rider": all_dices[big_slice, metric_axis[4]].mean(),
                     "vehicles": all_dices[big_slice, metric_axis[5]].mean(),
                     "bicycle": all_dices[big_slice, metric_axis[6]].mean(),
                     "motorcycle": all_dices[big_slice, metric_axis[7]].mean(),
                     "traffic_sign": all_dices[big_slice, metric_axis[8]].mean() } if len(metric_axis) > 1 else {}
...
def run(args: argparse.Namespace) -> None:
    ...
    for i in range(n_epoch):
        ...
        df = pd.DataFrame({"tra_loss": metrics["tra_loss"].mean(dim=1).cpu().numpy(),
                           ...
                           "tra_IoU": metrics["tra_dice"][:, :, -1].mean(dim=1).cpu().numpy(),
                           "val_IoU": metrics["val_dice"][:, :, [-1,-2]].mean(dim=1).mean(dim=1).cpu().numpy(),
                           #"tra_batch_dice": metrics["tra_batch_dice"][:, :, -1].mean(dim=1).cpu().numpy(),
                           #"val_batch_dice": metrics["val_batch_dice"][:, :, -1].mean(dim=1).cpu().numpy(), 
                           'building':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,0], 
                           'fence':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,1],
                           'other':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,2],
                           'pedestrian':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,3],
                           'pole':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,4],
                           'road line':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,5],
                           'road':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,6],
                           'sidewalk':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,7],
                           'vegetation':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,8] ,
                           'four-wheeler vehicle':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,9] ,
                           'wall':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,10] ,
                           'traffic sign':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,11] ,
                           'sky':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,12] ,
                           'ground':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,13] ,
                           'bridge':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,14] ,
                           'rail track':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,15] ,
                           'guard rail':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,16] ,
                           'traffic light':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,17] ,
                           'water':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,18] ,   
                           'terrain':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,19] ,     
                           'two-wheeler':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,20] , 
                           'static':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,21] ,             
                           'dynamic':  metrics["val_dice"][:, :, range(0,25)].mean(dim=1).cpu().numpy()[:,22] ,             
                           })
        ...
        if current_dice > best_dice:
            ...
            if best_folder.exists():
                rmtree(best_folder)
            copytree(Path(savedir, "buffer"), Path(best_folder))
        ...
        if i > 0 and not (i % 5):
            ...
            print(f"> Best results at epoch {best_epoch}: IoU: {best_dice:.3f}{maybe_hauss}")



def get_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Hyperparams')
    parser.add_argument('--dataset', type=str, default='/home/2017011/reljur01/OMNIDATA/FOLD_V2')
    parser.add_argument("--csv", type=str, default= 'metrics.csv')
    parser.add_argument("--workdir", type=str, default = '/home/2017011/reljur01/OMNIDATA/FOLD_V2/results/ResidualUNet')
    ...
    parser.add_argument('--n_epoch', nargs='?', type=int, default=45,
                        help='# of the epochs')
    ...
    parser.add_argument("--metric_axis", type=int, nargs='+', help="Classes to display metrics", default=range(1,10))
    ...
    parser.add_argument("--losses", type=str, 
                        ...
                        default="[('', {'idc': [0, 1], 'mode': 'multiclass','alpha': 0.5,'gamma': 2.,'ignore_index': [0]}, None, None, None, 1)]")
    '''
    
    parser.add_argument("--losses", type=str, 
                        help="List of (loss_name, loss_params, bounds_name, bounds_params, fn, weight)", 
                        default="[('CrossEntropy', {'idc': range(0,10)}, None, None, None, 1)]")
    
    
    parser.add_argument("--folders", type=str,
                        help="List of (subfolder, transform, is_hot)", default="[('rgb_images',png_transform, False), ('gtLabels', gt_transform, True)]+[('gtLabels', gt_transform, True)]" )
